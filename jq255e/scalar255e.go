...
// This file defines types and functions for jq255e scalars, i.e.
...
// Jq255eScalar is the type for an integer modulo the prime order of the
// jq255e group. Default value is zero.
...
func (s *Jq255eScalar) Decode(src []byte) int {
	return scalar.Decode((*[4]uint64)(s), src, &jq255eOrder)
}
...
func (s *Jq255eScalar) DecodeReduce(src []byte) {
	scalar.DecodeReduce((*[4]uint64)(s), src, jq255eModrReduce384Partial)
}
...
func (s *Jq255eScalar) Encode(dst []byte) []byte {
	return scalar.Encode(dst, (*[4]uint64)(s), jq255eModrReduce256)
}

// Encode a scalar into exactly 32 bytes.
func (s *Jq255eScalar) Bytes() [32]byte {
	return scalar.ToBytes((*[4]uint64)(s), jq255eModrReduce256)
}
...
func (s *Jq255eScalar) IsZero() int {
	var t [4]uint64
	jq255eModrReduce256(&t, (*[4]uint64)(s))
	z := t[0] | t[1] | t[2] | t[3]
	return int(1 - ((z | -z) >> 63))
}
...
func (s *Jq255eScalar) Add(a, b *Jq255eScalar) *Jq255eScalar {
	scalar.Add((*[4]uint64)(s), (*[4]uint64)(a), (*[4]uint64)(b), jq255eModrReduce256Partial)
	return s
}
...
func (s *Jq255eScalar) Sub(a, b *Jq255eScalar) *Jq255eScalar {
	scalar.Sub((*[4]uint64)(s), (*[4]uint64)(a), (*[4]uint64)(b), jq255eModrReduce256Partial, &jq255eOrder)
	return s
}
...
func (s *Jq255eScalar) Neg(a *Jq255eScalar) *Jq255eScalar {
	...
	scalar.Sub((*[4]uint64)(s), &t, (*[4]uint64)(a), jq255eModrReduce256Partial, &jq255eOrder)
	return s
}
...
func (s *Jq255eScalar) Mul(a, b *Jq255eScalar) *Jq255eScalar {
	scalar.Mul((*[4]uint64)(s), (*[4]uint64)(a), (*[4]uint64)(b), jq255eModrReduce384Partial)
	return s
}

// Group order is r = 2^254 - r0, with:
//    r0 = 131528281291764213006042413802501683931
const jq255e_r0_lo uint64 = 0xE0AD37518B27BADB
const jq255e_r0_hi uint64 = 0x62F36CF0ABF873AC

var jq255eOrder = [4]uint64 {
	0x1F52C8AE74D84525,
	0x9D0C930F54078C53,
	0xFFFFFFFFFFFFFFFF,
	0x3FFFFFFFFFFFFFFF,
...
func jq255eModrReduce256PartialWithExtra(d, a *[4]uint64, ah uint64) {
	// Truncate to 254 bits and get extra bits into ah.
	ah = (ah << 2) | (a[3] >> 62)

	// Compute ah*r0 into u0:u1:u2.
	u1, u0 := bits.Mul64(ah, jq255e_r0_lo)
	u2, lo := bits.Mul64(ah, jq255e_r0_hi)
	var cc uint64
...
// less than 2*r.
func jq255eModrReduce256Partial(d, a *[4]uint64) {
	jq255eModrReduce256PartialWithExtra(d, a, 0)
}

// Given a partially reduced input 'a' (less than 2*r), finish reduction
// (conditional subtraction of r).
func jq255eModrReduce256Finish(d, a *[4]uint64) {
	// Subtracting r is equivalent to adding r0, and subtracting
	// 2^254.
	var t [4]uint64
	var cc uint64
	t[0], cc = bits.Add64(a[0], jq255e_r0_lo, 0)
	t[1], cc = bits.Add64(a[1], jq255e_r0_hi, cc)
...
func jq255eModrReduce256(d, a *[4]uint64) {
	jq255eModrReduce256Partial(d, a)
	jq255eModrReduce256Finish(d, d)
}

// Given a 384-bit input 'a', perform a partial reduction modulo r;
// output fits on 255 bits and is less than 2*r.
func jq255eModrReduce384Partial(d *[4]uint64, a *[6]uint64) {
	...
	jq255eModrReduce256PartialWithExtra(d, &tw, th)
}

// Given an input k (fully reduced, in 0..r-1) and e < 2^127-2,
// compute d = round(k*e / r)
func jq255eMulDivrRounded(d *[2]uint64, k *[4]uint64, e *[2]uint64) {
	// z <- k*e
	var z [6]uint64
...
var jq255e_u = [2]uint64 { 0x2ACCF9DEC93F6111, 0x1A509F7A53C2C6E6 }
var jq255e_v = [2]uint64 { 0x0B7A31305466F77E, 0x7D440C6AFFBB3A93 }
...
func (k *Jq255eScalar) SplitMu(k0, k1 *[2]uint64) {
	...
	var t [4]uint64
	jq255eModrReduce256(&t, (*[4]uint64)(k))

	// c = round(k*v / r)
	// d = round(k*u / r)
	var c, d [2]uint64
	jq255eMulDivrRounded(&c, &t, &jq255e_v)
	jq255eMulDivrRounded(&d, &t, &jq255e_u)

	// k0 = k - d*u - c*v
	var y [2]uint64
	var cc uint64
	scalar.Mul128x128trunc(&y, &d, &jq255e_u)
	t[0], cc = bits.Sub64(t[0], y[0], 0)
	t[1], _ = bits.Sub64(t[1], y[1], cc)
	scalar.Mul128x128trunc(&y, &c, &jq255e_v)
	k0[0], cc = bits.Sub64(t[0], y[0], 0)
	k0[1], _ = bits.Sub64(t[1], y[1], cc)

	// k1 = d*v - c*u
	scalar.Mul128x128trunc(k1, &d, &jq255e_v)
	scalar.Mul128x128trunc(&y, &c, &jq255e_u)
...
func (a *Jq255eScalar) recode5(d *[52]byte) {
