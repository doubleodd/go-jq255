...
// This file implements high-level operations over jq255s:
//
//   - Key pair generation
//   - Key exchange (ECDH)
...
// scalar for jq255s. For efficiency reasons, it internally caches a
// copy of the public key as well.
type Jq255sPrivateKey struct {
	d Jq255sScalar
	pub Jq255sPoint
	epub [32]byte
}
...
type Jq255sPublicKey struct {
	pub Jq255sPoint
	epub [32]byte
}

// Test whether a public key is equal to another.
func (pk Jq255sPublicKey) Equal(other crypto.PublicKey) bool {
	pk2, ok := other.(Jq255sPublicKey)
	if !ok {
		return false
	}
	var t byte = 0
...
// non-zero scalar for jq255s, then this function returns nil and an
// error.
func Jq255sDecodePrivateKey(src []byte) (*Jq255sPrivateKey, error) {
	if len(src) != 32 {
		return nil, errors.New("Invalid private key")
	}
	sk := new(Jq255sPrivateKey)
...
// private key.
func (sk *Jq255sPrivateKey) Encode(dst []byte) []byte {
	return sk.d.Encode(dst)
}

// Get the public key corresponding to a given private key.
func (sk *Jq255sPrivateKey) Public() *Jq255sPublicKey {
	pk := new(Jq255sPublicKey)
	pk.pub.Set(&sk.pub)
...
// 32 bytes. If the provided slice does not have length exactly 32,
// or if what it contains is not the canonical encoding of a valid
// non-neutral jq255s element, then this function returns nil and an
// error.
func Jq255sDecodePublicKey(src []byte) (*Jq255sPublicKey, error) {
	if len(src) != 32 {
...
// the concatenation of the current contents of 'dst' and the encoded
// public key.
func (pk *Jq255sPublicKey) Encode(dst []byte) []byte {
	n := len(dst)
	n2 := n + 32
	var dst2 []byte
	if cap(dst) >= n2 {
...
// Key pair generation with jq255s: from a random source 'rand', a
...
// the recommended way).
func Jq255sGenerateKeyPair(rand io.Reader) (*Jq255sPrivateKey, error) {
...
// Key exchange with jq255s: given our private key, and the public key
...
// it is not otherwise a valid jq255s point encoding, then the key
...
func Jq255sKeyExchange(sk *Jq255sPrivateKey, peer_pk []byte) (secret [32]byte, ok int) {
...
// Schnorr signature with jq255s. The data to sign ('data') may be either
...
func (sk *Jq255sPrivateKey) signWithSeed(seed []byte, data []byte, opts crypto.SignerOpts) (signature []byte, err error) {
	// Get hash label.
	var hashLabel []byte
...
func (sk *Jq255sPrivateKey) Sign(rand io.Reader, data []byte, opts crypto.SignerOpts) (signature []byte, err error) {
	if rand == nil {
...
func (pk *Jq255sPublicKey) VerifyVartime(data []byte, opts crypto.SignerOpts, sig []byte) bool {
...
func Jq255sHashToCurve(data []byte, opts crypto.SignerOpts) (*Jq255sPoint, error) {
	...
	P1.MapBytes(bb1[:])
	P2.MapBytes(bb2[:])
	return NewJq255sPoint().Add(&P1, &P2), nil
}
